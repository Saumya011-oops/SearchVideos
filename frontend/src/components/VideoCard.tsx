...
function StatusBadge({ video }: { video: Video }) {
  ...
  const badgeStyles: Record<string, React.CSSProperties> = {
    processing: { background: 'rgba(226, 179, 64, 0.2)', color: '#c49a2a', border: '1px solid rgba(226, 179, 64, 0.4)' },
    ready: { background: 'rgba(45, 106, 106, 0.15)', color: '#2d6a6a', border: '1px solid rgba(45, 106, 106, 0.3)' },
    failed: { background: 'rgba(212, 96, 58, 0.15)', color: '#d4603a', border: '1px solid rgba(212, 96, 58, 0.3)' },
    uploading: { background: 'rgba(100, 100, 100, 0.1)', color: '#666', border: '1px solid rgba(100, 100, 100, 0.2)' },
  }
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      <span
        style={{
          ...badgeStyles[video.status], fontSize: '11px', fontWeight: 500, padding: '3px 8px',
      ...
      >
        {video.status === 'processing' && (
          <span style={{
            width: '6px', height: '6px', borderRadius: '50%', background: '#e2b340',
      ...
      {showTooltip && video.error_message && (
        <div style={{
          ...
          marginBottom: '6px', background: '#1a1a2e', color: '#f5f0e8', fontSize: '11px',
          padding: '6px 10px', borderRadius: '6px', maxWidth: '200px', whiteSpace: 'normal',
...
export default function VideoCard({ video, onClick, onDeleted }: VideoCardProps) {
  ...
  return (
    <div
      ...
      style={{
        background: '#fff', borderRadius: '10px', position: 'relative',
        boxShadow: hovered ? '0 8px 32px rgba(226, 179, 64, 0.18)' : '0 4px 24px rgba(226, 179, 64, 0.10)',
    ...
    >
      ...
      {hovered && (
        <button
          ...
          style={{
            ...
            background: confirmDelete ? '#d4603a' : 'rgba(26,26,46,0.75)',
      ...
      {/* Thumbnail */}
      <div style={{
        height: '160px', background: 'linear-gradient(135deg, #1a1a2e 0%, #2d3050 100%)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        position: 'relative', overflow: 'hidden',
      }}>
        {video.thumbnail_path ? (
          <img src={video.thumbnail_path} alt={video.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
        ...
        <div style={{
          ...
          background: 'rgba(26, 26, 46, 0.85)', color: '#f5f0e8',
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        <h3 className="heading" style={{
          color: '#1a1a2e', fontSize: '16px', fontWeight: 400, margin: '0 0 8px',
